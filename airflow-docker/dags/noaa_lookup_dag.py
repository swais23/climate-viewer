...
from queries.noaa_lookup_checks import (
    countries_freshness,
    states_freshness,
    stations_freshness,
)
...
def noaa_lookup(lookup_config) -> None:
    ...
    column_specs = [(column.start_position, column.end_position) for column in lookup_config.column_definition]
    ...
    formatted_query = noaa_lookup_query.format(table=lookup_config.table_name, columns=", ".join(column_names))
...
with DAG(
    ...
    lookup_groups = {}
    freshness_queries = {"stations": stations_freshness, "countries": countries_freshness, "states": states_freshness}
    ...
    for config in NOAA_LOOKUP_CONFIG:
        ...
        with TaskGroup(f"{table}_group", tooltip=f"{table}_group") as lookup_group:
            ...
            nothing_to_load = DummyOperator(task_id=f"{table}_nothing_to_load")

            lookup_load = PythonOperator(task_id=f"{table}_lookup_load", python_callable=noaa_lookup, op_args=[config])
